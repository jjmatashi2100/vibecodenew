...
export function Stage4StyleGuide() {
  ...
  const { saveStageData, getContextForStage, workflowPrev, acceptStage, currentProject } =
    useProjectStore();
  ...
  /* ------------------------------------------------------------------
     Hydrate preferences / output from autosave on mount or project change
  -------------------------------------------------------------------*/
  useEffect(() => {
    const auto = currentProject?.settings?.autosave?.stage4;
    if (auto) {
      if (auto.designPreferences && !designPreferences)
        setDesignPreferences(auto.designPreferences);
  ...
  /* ------------------------------------------------------------------
     Autosave every 10 s whenever designPreferences/output change
  -------------------------------------------------------------------*/
  useEffect(() => {
    if (!currentProject?.id) return;
    if (!designPreferences.trim() && !output.trim()) return;
  ...
  }, [designPreferences, output, currentProject?.id]);
  ...
  async function evaluateOutput() {
    ...
    setEvalResult(null);
    try {
      const prompt = STAGE_PROMPTS.stage4.evaluate(output);
      ...
      if (parsed) {
        ...
        await saveStageData(4, { content: output, feedback: parsed });
      }
    } catch (e) {
  ...
  async function applyOptimizations() {
    ...
    try {
      const prompt = STAGE_PROMPTS.stage4.optimize(output, evalResult.deltas);
      ...
      if (typeof improved === 'string') {
        setOutput(improved);
        setEvalResult(null);
        await saveStageData(4, { content: improved, feedback: evalResult });
      }
    } catch (e) {
